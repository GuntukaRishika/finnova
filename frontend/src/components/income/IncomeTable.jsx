...
function IncomeTable({ page, isLoading, onEdit, onDelete, onPageChange }) {
            ...
            {!isLoading &&
              content.map((income) => (
                <tr key={income.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 text-slate-600">{formatDate(income.incomeDate)}</td>
                  <td className="px-6 py-4">
                    <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                      {income.categoryName}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-600">{income.description || '—'}</td>
